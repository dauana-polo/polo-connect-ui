...
import { AlertCircle, CheckCircle2, FileDown, Link2, Search, Trophy, UserPlus, XCircle } from "lucide-react";
  ...
  return (
    <div className="space-y-3 pt-2">
      <div className="flex flex-wrap items-start justify-between gap-2 rounded-md border p-3">
        <div><h3 className="text-sm font-semibold">Fluxo comercial desta negociação</h3><p className="text-xs text-muted-foreground">Cliente, contatos, recomendação, proposta e fechamento permanecem neste card.</p></div>
        ...
        <Tabs defaultValue="cliente">
          ...
          <TabsContent value="cliente" className="grid gap-4 pt-4 lg:grid-cols-2">
            <Card><CardHeader><CardTitle className="text-base">Buscar e incluir cliente</CardTitle></CardHeader><CardContent className="space-y-3"><div className="relative"><Search className="absolute left-3 top-3 h-4 w-4 text-muted-foreground" /><Input className="pl-9" defaultValue="Nexa" aria-label="Buscar cliente por nome ou telefone" /></div>{["Nexa Tecnologia · (11) 98765-4321", "Nexa Consultoria · sem CNPJ"].map((name) => <Button key={name} variant="outline" className="h-auto w-full justify-between whitespace-normal p-3 text-left" onClick={() => toast.success("Cliente seria incluído nesta negociação.")}><span>{name}</span><Link2 className="h-4 w-4 shrink-0" /></Button>)}<Button variant="secondary" className="w-full" onClick={() => toast.info("Preencha o cadastro rápido ao lado para criar e incluir o cliente.")}><UserPlus className="h-4 w-4" /> Criar novo cliente</Button></CardContent></Card>
            <Card><CardHeader><CardTitle className="text-base">Criar e incluir no negócio</CardTitle></CardHeader><CardContent className="space-y-3"><div className="grid gap-3 sm:grid-cols-2"><Input placeholder="Nome do cliente" /><Input placeholder="Telefone" /><Input placeholder="Nome do contato" /><Input placeholder="Cargo" /><Input placeholder="E-mail" /><Input placeholder="Telefone do contato" /></div><label className="flex items-center gap-2 text-sm"><Checkbox defaultChecked /> Salvar cliente sem CNPJ por enquanto</label><div className="rounded-md border border-warning/30 bg-warning/10 p-3 text-xs"><AlertCircle className="mr-2 inline h-4 w-4" />O CNPJ será obrigatório no fechamento.</div><Button className="w-full" onClick={() => toast.success("Cliente e contato seriam criados e incluídos nesta negociação.")}><UserPlus className="h-4 w-4" /> Criar e incluir cliente</Button></CardContent></Card>
          </TabsContent>
